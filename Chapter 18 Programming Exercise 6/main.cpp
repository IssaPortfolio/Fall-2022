...
template <class Type>
void sortedInsert(stackType<int> primeNumbersStack, int x) {
	if ((primeNumbersStack.isEmptyStack()) || (x > primeNumbersStack.top()))
		primeNumbersStack.push(x);
	sortedInsert<int>(primeNumbersStack, x);
	primeNumbersStack.push(primeNumbersStack.pop());
}

template <class Type>
void sortStack(stackType<int> primeNumbersStack) {
	if (!primeNumbersStack.isEmptyStack()) {
		sortStack<int>(primeNumbersStack);
	}
}
...
int main()
{
 ...
 //of stack into otherStack
	...
	stackType<int> primeNumbersStack;
	assert(number >= 0);
	for (int p = 2; p <= number; p++) {
		if (number % p == 0) {
			...
			if (prime)
				primeNumbersStack.push(p);
		}
	}
	while (!primeNumbersStack.isEmptyStack()) {
		cout << primeNumbersStack.top() << endl;
		primeNumbersStack.pop();
	}
