...
template <class Type>
void linkedListType<Type>::divideAt(linkedListType<Type>& subList, const Type& item) {
	...
	if (current->info != item)
		while ((current->link != nullptr) && (current->link->info != item))
			current = current->link;

	subList.initializeList();
	nodeType<Type>* current1 = current->link;
	current->link = nullptr;
	last = current;

	while (current1 != nullptr){
		subList.insertLast(current1->info);
